...
var bubbleuniverse = new Supersystem()
////////////////GAME LOOP/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
////////update function this is the game loop bruh///////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
function update() {
	time = time + 1;
	var vwx = systems[ps].players[0].ship.x;
	var vwy = systems[ps].players[0].ship.y;
	var myplayer = systems[ps].players[0];
	var myship = systems[ps].players[0].ship; 
	...
	if (ps==0){
		myship.x = 0;
		myship.y = -100;//temp to avoid spawning on inactive bombs
		myship.vx = 0;
		myship.vy = 0;
...
//Outpost stuff all handled here.....
	...
	while (i<systems[ps].outposts.length){ //for all outposts
		if (myship.distance(systems[ps].outposts[i])<160){
			systems[ps].outposts[i].dock(myship);
			myplayer.dockstate = i;
			}
		var j= 0;
...
//////////////////////////////////Weapon 6 in-loop code, its special like that////////////////////////////////////////////////////////////////////////////////////
	if((myplayer.mousestate==1)&&(myplayer.wep == 6)&&(myplayer.energy>2)){ 
		...
		var range = myplayer.blasters[6].timer+32;
		context.beginPath();
		context.lineWidth = 12; 
		context.moveTo(canvas.width/2+Math.cos(myship.d)*32, canvas.height/2+Math.sin(myship.d)*32);
		context.lineTo(canvas.width/2+Math.cos(myship.d)*range,canvas.height/2+Math.sin(myship.d)*range);
