function optionsvkeys(xmax,ymax){   //constructor(key,label,x,y){
	...
	var mhd=new Virtualkey(15,"Mini HUD toggle",250,432);//constructor(key,label,x,y)
	var cthrust=new Virtualkey(16,"Toggle Continuous Thrust",450,368);//constructor(key,label,x,y)
	var res=new Virtualkey(17,"Reset resolution",450,400);//constructor(key,label,x,y)
	var exit=new Virtualkey(18,"Click here or press o to exit",250,520);//constructor(key,label,x,y)
	return [mvp,mvm,svp,svm,ss,sta,vka,vkv,fup,fum,ttt,myp,mym,tmt,tmu,mhd,cthrust,res,exit];
}
function optionsactions(action,theplayer){
	switch(action){
		...
		case 14:
			...
			break;
		case 15:
			console.log("triedtocomfytoggle");
			console.log(comfy);
			if (comfy==true){
				comfy = false;
			...
			break;
		case 16:
			theplayer.cthrust = !theplayer.cthrust;
			break;
		case 17:
