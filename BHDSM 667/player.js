class Player{
	...
	addspark(){
		var aspark = new Umo(0,0,12,"red");//	constructor(xxx, yyy, sss, ccc) {
		aspark.match(this.ship);
		var randomvert = Math.floor(this.ship.polytheta.length*Math.random());
		aspark.x = aspark.x+Math.cos(this.ship.polytheta[randomvert])*(this.ship.polyradius[randomvert]*(this.ship.s)+12);
		aspark.y = aspark.y+Math.sin(this.ship.polytheta[randomvert])*(this.ship.polyradius[randomvert]*(this.ship.s)+12);
		aspark.push(Math.random()+1,this.ship.polytheta[randomvert]);
		aspark.timer = 24;
		this.sparks.push(aspark);
		}
	updatesparks(){
		if (this.ship.damagestate>0){
			this.addspark();
			}
		var i = 0;
		while(i<this.sparks.length){
			this.sparks[i].c = rainbow(Math.floor(this.sparks[i].timer/2+3)%6);//The +3 is very optional.
			this.sparks[i].update1();
			this.sparks[i].timer--;
			this.sparks[i].s = Math.floor(this.sparks[i].timer/2);
			if (this.sparks[i].timer<=0){
				this.sparks.splice(i, 1);//remove it
				}
			else{
				i++;
				}
	...
	drawsparks(viewx,viewy){
		var i=0;
		while(i<this.sparks.length){
			this.sparks[i].drawdot(viewx,viewy);
			i++;
			}
		return;
		}
	drawmessages(time){//This draws messages from the stack as appropriate;
		var currentmessages = [];
		var i=0;
		while(i<this.messages.length){
			if ((this.messges[i].start>time)&&(this.messages[i].end<time)){
				currentmessages.push(this.messages[i]);
				}
			i++;
			}
		var i=0;
		while(i<currentmessages.length){
			context.fillStyle = currentmessages[i].c;
			context.font = "16px Ariel"
			context.fillText(currentmessages[i].text,currentmessages[i].x,currentmessages[i].y);
			i++;
			}
		return;
		}
    ...
    update1(theplanets){
		...
		if (this.alive==false){
			...
			else{//This is a bunch of things needed to respawn properly
				this.deadscore = 0;
				this.alive = true;
				this.ship.hp = this.ship.maxhp;
				console.log(this.ship.hp);
		...
		if ((this.autozoom==1)&&(this.navactive==1)){
			console.log("!!");
			var targetdistance = this.ship.distance(systems[ps].planets[this.navtarget]);
			if (targetdistance>this.mapscale*200){
				this.mapscale = targetdistance/100;//this.mapscale*1.1;
				}
			else if ((targetdistance<this.mapscale*50)&&(this.mapscale>32)){
	...
	damagesounds(){
		if (this.ship.damagestate == 3){
			if (!armorhit0.paused) { armorhit0.pause(); }
			armorhit0.currentTime = 0;
			armorhit0.play();
			}
		if (this.ship.shielddamagestate == 3){
