class Umo { //Universal Moving Object
	...
	drawplanet(viewx, viewy){ //input variables are player ship/camera position
		var x = this.x - viewx + canvas.width/2; //onscreen coordinates
		var y = this.y - viewy + canvas.height/2; 
		 //this function draws object as a circle, and labels it
		context.beginPath();
	...
	updateship(theplanets){//Unfortunately I need access to the list of planets to handle ship respawning.
		//dont think I need theplanets anymore...
		...
		if ((this.cthrust > 0)&&(systems[ps].players[myi].thruster>0)){//shame, global scope
			console.log("cthrustin");
			this.vx = this.vx + this.cthrust*Math.cos(this.d);
			this.vy = this.vy + this.cthrust*Math.sin(this.d);
			systems[ps].players[myi].thruster = systems[ps].players[myi].thruster - 2;//shame, global scope
			this.thruststate = 3;
			}
