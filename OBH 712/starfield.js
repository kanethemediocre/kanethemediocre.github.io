	...
	draw(viewx,viewy,viewz){
		...
		while (i<this.starlayers.length){
			...
			while(j<this.starlayers[i].length){
				//var dxraw = this.starlayers[i][j].x-(viewx / xyscale) ;
				//if (dxraw>(this.xmax/xyscale)){var dx = (dxraw%(this.xmax/xyscale))/dz;}
				//else if (dxraw<-1*this.xmax/xyscale){var dx = -1*( (-1*dxraw)%(this.xmax/xyscale) ); }
				//else { var dx = dxraw/dz; }//(this.starlayers[i][j].x-(viewx / xyscale))/dz ;
				//var dx = dxraw/dz
...
class Starfield3{
	...
	draw(viewx,viewy,viewz){
		...
		while (i<this.starlayers.length){
			...
			while(j<this.starlayers[i].length){
				var dxraw = this.starlayers[i][j].x-(viewx / xyscale) ;
				if (dxraw>this.xmax){var dx = (dxraw%this.xmax)/dz;}
				else if (dxraw<-1*this.xmax){var dx = -1*( (-1*dxraw)%this.xmax ); }
				else { var dx = dxraw/dz; }//(this.starlayers[i][j].x-(viewx / xyscale))/dz ;
				if (Math.abs(dx)<hcx){//This is sort of an optimization?  
					var dy = ( (this.starlayers[i][j].y-(viewy / xyscale))/dz );
					if (Math.abs(dy)<hcy){
