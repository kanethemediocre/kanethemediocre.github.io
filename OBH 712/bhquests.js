...
var t0q0reqs = [new Questrequirement("pasttime",[],[200,200])];
var startmsg = "";
var endmsg = "Don't touch anything just yet.  Some people think all you need to know is right click zoom left click boom, but getting anywhere in particular is more complicated.  Learn to use your ship's map and nav computer, or you'll find what space is really made made of:  Nothing.";
var thenpcname = "Merriweather"
t0quests.push(new Quest(thenpcname,startmsg,endmsg,0,t0q0reqs));

var t0q1reqs = [new Questrequirement("pasttime",[],[200,0])];//qvar[1] is set to 0, but will be corrected when advancing to the quest.
var startmsg = "";
var endmsg = "The N key will activate or retask your nav computer, which displays on the bottom right.  The < and > keys will cycle nav targets or you can hit Q and click a planet from the quick select menu.  Use either to select Merz.  The nav computer indicates much more than direction and distance if you're interested, but most of the time that's all you need.";
t0quests.push(new Quest(thenpcname,startmsg,endmsg,0,t0q1reqs));

var t0q2reqs = [new Questrequirement("selectnavtarget",[5,1],[])];
var startmsg = "";
var endmsg = "Great!  You'll be going there soon.  But to help you know your place in the universe, your HUD also has the Omnicompass and a map.  The Omnicompass can be activated or retasked with the C key, and the map with the M key. The map features a full-screen display mode and a corner display mode.  The Omnicompass tracks planets, ships, or both, and is good for staying aware of things just outside of visual range.";
t0quests.push(new Quest(thenpcname,startmsg,endmsg,0,t0q2reqs));

var t0q3reqs = [new Questrequirement("pasttime",[],[200,0])];//qvar[1] is set to 0, but will be corrected when advancing to the quest.
var startmsg = "";
var endmsg = "The yellow rectangle in the center of the map represents the area that you can see on the screen without the map.  Planets, moons, and stations are drawn on the map, and planetary orbits are shown with grey lines.   You can zoom in and out with the + and - keys.  Try zooming way out (- key) so that the whole system is in view.";
t0quests.push(new Quest(thenpcname,startmsg,endmsg,0,t0q3reqs));

var t0q4reqs = [new Questrequirement("minzoom",[],[1024])];
var startmsg = "";
var endmsg = "Nifty, right?  OK, so we're not going nearly that far away, so zoom back in on us and Merz. The number 1 struggle for new pilots is stopping at the destination, which requires about as much thrust as getting there usually.  Some pilots count their pulses, and know that they need that many in the opposite direction to stop.";
t0quests.push( new Quest(thenpcname,startmsg,endmsg,0,t0q4reqs));

var t0q5reqs = [new Questrequirement("maxzoom",[],[128])];
var startmsg = "";
var endmsg ="The map and nav computer are more than sufficient guide you wherever you need, but some people like the Omnicompass (C key) because it tracks everything at once.  OK, so try applying what you've learned to fly to Merz.  Don't get in any fights, just use your thrusters (right click) to do a flyby and come back.";var t0q5 = new Quest(thenpcname,startmsg,endmsg,0,t0q5reqs);
t0quests.push(new Quest(thenpcname,startmsg,endmsg,0,t0q5reqs));

var t0q6reqs = [new Questrequirement("nearplanet",[4],[500,10])];
var startmsg = "";
var endmsg =  "Now get back to the Merry Merzian.  Remember you can use your nav (N, <, >) computer to find the station.  When you're close you'll dock automatically.  If you're far away, hit the  A key to activate the autopilot, which will get you closer, then hit A again when you're close to resume manual flight.";
t0quests.push(new Quest(thenpcname,startmsg,endmsg,0,t0q6reqs));

var t0q7reqs = [new Questrequirement("dockedstation",[],[200,0])];//qvar[1] is set to 0, but will be corrected when advancing to the quest.
var startmsg = "";
var endmsg = "The autopilot works well between distant objects in empty space, and most trips you'll want it for fit that description well enough, but you will need to guide your ship in manually for the last few thousand meters.  Most planets and moons have hostile Umobots.  They orbit their host planets and shoot at any ships that come near.  Around Merz they aren't particularly dangerous, but each planet has it's own breed of bot, and most are more dangerous than the Merz bots.";
t0quests.push(new Quest(thenpcname,startmsg,endmsg,0,t0q7reqs));

var t0q8reqs = [new Questrequirement("pasttime",[],[200,0])];//qvar[1] is set to 0, but will be corrected when advancing to the quest.
var startmsg = "";
var endmsg = "Bots can be destroyed easily enough, but replacements are never far behind.  Still, the Institute pays out for each one destroyed, as well as for the resulting salvage.  That salvage, called Bling, is so universally valued it has become currency.";
t0quests.push(new Quest(thenpcname,startmsg,endmsg,0,t0q8reqs));

var t0q9reqs = [new Questrequirement("pasttime",[],[200,0])];//qvar[1] is set to 0, but will be corrected when advancing to the quest.
var startmsg = "";
var endmsg = "No one knows where they came from, or why they suddenly started appearing 30 years ago.  But life off-world has changed dramatically since they came.  Most of your ships systems are made from Bling technology, and the Mississippi Institute of Science Services Inside Sool Space Including Protection, Profit, and Intelligence built a whole new industry around both fighting and studying the bots.";
t0quests.push( new Quest(thenpcname,startmsg,endmsg,0,t0q9reqs));

var t0q10reqs = [new Questrequirement("pasttime",[],[200,0])];//qvar[1] is set to 0, but will be corrected when advancing to the quest.
var startmsg = "";
var endmsg = "You better learn a thing or two about combat before I send you anywhere more dangerous, eh?  The top left corner of your HUD has a few important indicators.  At the top the green bar indicates your ship's health.  It's a basic measure of how close you are to blowing up.  Don't blow up.";
t0quests.push(new Quest(thenpcname,startmsg,endmsg,0,t0q10reqs));

var t0q11reqs = [new Questrequirement("pasttime",[],[200,0])];//qvar[1] is set to 0, but will be corrected when advancing to the quest.
var startmsg = "";
var endmsg = "The next blue bar is your shield strength.  Your shield is a fast regenerating energy barrier that prevents your ship from being damaged by occasional minor hits.  Don't enter combat without a full shield, and get out if it ever goes down to 0.  A weak shield will still reduce the damage you take compared to no shield at all.";
t0quests.push( new Quest(thenpcname,startmsg,endmsg,0,t0q11reqs));

var t0q12reqs = [new Questrequirement("pasttime",[],[200,0])];//qvar[1] is set to 0, but will be corrected when advancing to the quest.
var startmsg = "";
var endmsg = "The purple bar is your ship level, which approximately represents your ship's value.  The next red bar is your weapons energy.  The ship has capacitors to provide the burst current that the ships reactor can't directly provide.  The red bar indicates their energy level.";
t0quests.push( new Quest(thenpcname,startmsg,endmsg,0,t0q12reqs));

var t0q13reqs = [new Questrequirement("pasttime",[],[200,0])];//qvar[1] is set to 0, but will be corrected when advancing to the quest.
var startmsg = "";
var endmsg = "Finally, the orange bar at the bottom shows your thruster energy level.  Just like your weapons, the ship's thrust capacitors recharge quickly with power from the reactor.  Don't get caught in a fight without any go-juice to spare for escape or evasive maneuvers.";
t0quests.push( new Quest(thenpcname,startmsg,endmsg,0,t0q13reqs));

var t0q14reqs = [new Questrequirement("pasttime",[],[200,0])];//qvar[1] is set to 0, but will be corrected when advancing to the quest.
var startmsg = "";
var endmsg = "Below the status bars is a weapons indicator highlighting your current weapon in red, and showing available weapons in white (just weapon 1 for now).  You can fire most weapons with a left click, and use the scroll wheel to select different weapons if you have them.  A few weapons will require holding down the left mouse button.";
t0quests.push( new Quest(thenpcname,startmsg,endmsg,0,t0q14reqs));

var t0q15reqs = [new Questrequirement("pasttime",[],[200,0])];//qvar[1] is set to 0, but will be corrected when advancing to the quest.
var startmsg = "";
var endmsg = "The last part of the HUD we need to talk about is the targeting computer.  Your radar can detect enemies within a certain distance (4000p, as shown by the green circle on your map), and tell you useful information about them.  The targeting computer will track the last bot you pointed at, and add a color coded reticle if in visual range.";
t0quests.push( new Quest(thenpcname,startmsg,endmsg,0,t0q15reqs));

var t0q16reqs = [new Questrequirement("pasttime",[],[200,0])];//qvar[1] is set to 0, but will be corrected when advancing to the quest.
var startmsg = "";
var endmsg = "Not every ship out there is an enemy.  Use your radar and your targeting computer to tell friend from foe.  Speaking of which, why don't you do me a favor and destroy a particular bot?  The name is ";
t0quests.push( new Quest(thenpcname,startmsg,endmsg,0,t0q16reqs));

var t0q16reqs = [new Questrequirement("killnpc",[10],[])];
var startmsg = "Speaking of which, why don't you do me a favor and destroy a particular bot?  It should be near Merz.  Name is ";//Name should be filled in when qstate activates the quest 
var endmsg = "Alright good job.  Now before you get carried away come back to the station and I'll fit your ship with something a little more powerful.";;
t0quests.push( new Quest(thenpcname,startmsg,endmsg,0,t0q16reqs));

var t0q17reqs = [new Questrequirement("dockedstation",[1],[])];
var startmsg = "";
var endmsg = "Wait here for a moment while I work on your ship.  Your basic blaster, while not having any extraordinary capabilities, is energy efficient and effective for most situations.  The Mine launcher, is extraordinarily powerful but only useful in very special circumstances.";
t0quests.push( new Quest(thenpcname,startmsg,endmsg,0,t0q17reqs));

var t0q18reqs = [new Questrequirement("pasttime",[],[200,0]),new Questrequirement("dockedstation",[1],[])];
var startmsg = "";
var endmsg = "New weapons and upgrades can be purchased at stations like the mine.  I'm installing the mine launcher gratis, but don't count on any more freebies. ";
t0quests.push( new Quest(thenpcname,startmsg,endmsg,0,t0q18reqs));

var t0q19reqs = [new Questrequirement("pasttime",[],[200,0])];
var startmsg = "";
var endmsg = "Switch to weapon 2 and give it a shot.";
t0quests.push( new Quest(thenpcname,startmsg,endmsg,0,t0q19reqs));

var t0q20reqs = [new Questrequirement("bombout",[2],[])];
var startmsg = "";
var endmsg ="Watch how slowly it launches.  Don't fly into it!  The mine doesn't really get launched so much as dropped off with a gentle push.  It's up to to pilot to fly his ship to whatever momentum the mine needs, release and fly away.  The upshot is that the mine is both powerful and long-lasting.";
t0quests.push( new Quest(thenpcname,startmsg,endmsg,0,t0q20reqs));

var t0q21reqs = [new Questrequirement("pasttime",[],[200,0])];
var startmsg = "";
var endmsg ="Pilots sometimes will sometimes drop a mine and fly into combat behind it, or leave a mine in orbit of a planet in a target-rich environment.  Having a mine (or any bomb) active doesn't stop you from switching to and using other weapons without terminating the mine.";
t0quests.push( new Quest(thenpcname,startmsg,endmsg,0,t0q21reqs));

var t0q22reqs = [new Questrequirement("pasttime",[],[200,0])];
var startmsg = "";
var endmsg = "Although not usually used for picking off faraway targets, the mine arguably has the longest range (by far) of any weapon due to its long timer, provided the pilot provides a bit of speed for the mine.";
t0quests.push( new Quest(thenpcname,startmsg,endmsg,0,t0q22reqs));

var t0q23reqs = [new Questrequirement("pasttime",[],[200,0])];
var startmsg = "";
var endmsg = "Some weapons, including the mine, have an upgrade that allows you to detonate the bomb with a second click.  For the mine this is strongly recommended.";
t0quests.push( new Quest(thenpcname,startmsg,endmsg,0,t0q23reqs));

var t0q24reqs = [new Questrequirement("pasttime",[],[200,0])];
var startmsg = "";
var endmsg = "I don't sell that particular upgrade, and although I can install a remote upgrade for your basic blaster I don't recommend it.";
t0quests.push( new Quest(thenpcname,startmsg,endmsg,0,t0q24reqs));

var t0q25reqs = [new Questrequirement("pasttime",[],[200,0])];
var startmsg = "";
var endmsg = "Jojo's House of Cheese does have the upgrade, although it trails jupe far away from here.  Jojo knows everything about trading and cargo missions.";
t0quests.push( new Quest(thenpcname,startmsg,endmsg,0,t0q25reqs));

var t0q26reqs = [new Questrequirement("pasttime",[],[200,0])];
var startmsg = "";
var endmsg = "Bill's Billion Bits next door has a very helpful damage upgrade for your basic blaster.  Bill likes to show rookies the finer points of space combat.";
t0quests.push( new Quest(thenpcname,startmsg,endmsg,0,t0q26reqs));

var t0q26reqs = [new Questrequirement("pasttime",[],[200,0])];
var startmsg = "";
var endmsg = "I've got a blaster called the Railgun which is pretty good without any upgrades.  I've got more to teach about flying and navigating if you're interested.";
t0quests.push( new Quest(thenpcname,startmsg,endmsg,0,t0q26reqs));

//var t0quests = [t0q0,t0q1,t0q2,t0q3,t0q4,t0q5,t0q6,t0q7,t0q8,t0q9,t0q10,t0q11,t0q12,t0q13,t0q14,t0q15,t0q16,t0q17,t0q18,t0q19];
var Tutorial0 = new Questline(t0quests,0);
