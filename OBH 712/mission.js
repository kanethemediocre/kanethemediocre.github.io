	...
	check(theplayer, thenpcs,theplanets,theradio){ //Determines if mission is complete
		...
		if ((this.taken==true)&&(this.taker==theplayer.name)){ //This evaluates to false when this.taken == true.  Not sure why, but I need to fix it.
			...
			if (this.type == "destroy"){ //If the mission is to kill a guy
				if (thenpcs[this.target].ship.hp < 0){
					var playerid = 0; //assumption to be fixed later for multiple players
					var i = 0;
					while(i<thenpcs[this.target].ai.attackers.length){
						var killer = thenpcs[this.target].ai.attackers[i][0];
						if (killer == playerid+1000000){
							complete = 1;
							i = thenpcs[this.target].ai.attackers.length;
							}
						i++;
						}
						
					} //If the ship is dead, the mission is complete.  
				//Need a distance check, but the ship gets teleported on death.
				//var killer = thenpcs[this.target].ai.attackers[thenpcs[this.target].ai.attackers.length-1][0];
				}
