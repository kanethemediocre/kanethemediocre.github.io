...
class System{
	...
	draw2(viewx,viewy,myi){ //linear filtered instead of by distance, maybe computationally cheaper?
		//Before drawing objects, draw planetary orbit indicators, so they get covered by overlaps.
		var i=0;
		while  (i<this.planets.length){
			var sunx = this.planets[0].x - viewx + canvas.width/2; //onscreen coordinates
			var suny = this.planets[0].y - viewy + canvas.height/2; 
			//console.log("triedtodraworbitindicator"+i);
			if (this.planets[i].parentid==0){//If planet has sun for parentid, draw its orbit.
				//console.log("triedtodraworbitindicator");
				var psr = this.planets[i].distance(this.planets[0]);
				context.beginPath();
				context.strokeStyle = "rgb(255 255 0 / 25%)";
				//context.strokeStyle = "#rgb(92 64 51 / 25%)"; //sets color of orbit indicator
				var x = this.planets[0].x - viewx + canvas.width/2; //onscreen coordinates
				var y = this.planets[0].y - viewy + canvas.height/2; 
				context.arc(sunx, suny, psr, 0, 2 * Math.PI, false); //draws orbit indicator
				context.lineWidth = 2; //circle is thicc
				context.stroke();	//ok now actually draw it.
				}
			i++;
			}
		//Now start drawing all the things.
		//xtol, xdif, ytol, ydif are used to filter out objects out of screen space that don't need to be drawn.
		var i=0;
			...
			if (this.npcs[i].alive==true){
				...
				if ((xdif < xtol)&&(xdif > -1*xtol)){
					...
					if ((ydif < ytol)&&(ydif>-1*ytol)){
						...
						this.npcs[i].drawlevel(viewx,viewy);
						//this.npcs[i].drawdebug(viewx, viewy);
						//putting all the sparks stuff here.  If it's offscreen it's not worth simulating.
						if (this.npcs[i].ship.damagestate>0){
							this.npcs[i].addspark()
							}
						this.npcs[i].updatesparks();
						this.npcs[i].drawsparks(viewx,viewy);
						}
					}
				}
		...
		while  (i<this.planets.length){
			...
			//xtol, xdif, etc.  filter what planets need to be drawn
...
//drawmap(systems[ps].planets,systems[ps].outposts,theplayer.mapscale,200,canvas.height-150, vwx, vwy, theplayer.radarrange,systems[ps].npcs,theplayer);//corner map c 
		var mplanets = this.planets;
		var mstations = this.outposts;
		var scale = this.players[myi].mapscale;
		//var xx = 200;
		//var yy = canvas.height-150;
		var px = this.players[myi].ship.x;//vwx
		var py = this.players[myi].ship.y;//vwx
		var radar = this.players[myi].radarrange;
		var mnpcs = this.npcs;
		var theplayer = this.players[myi]
		...
		while (i>0){
			i = i-1;
			x = xx + mplanets[i].x/scale - px/scale;//x and y are on screen location of planet on map.
			...
			if (theplayer.sensor>-1){//Maybe <0 or <1 later, this threshold determines how many sensor levels needed to see orbits.  As is, player starts able to see orbits
				if (mplanets[i].parentid == 0){//If planet
					var oradius = mplanets[0].distance(mplanets[i])/scale;
					context.beginPath();
					context.strokeStyle = "darkslategrey"; //drawing faint orbit radius
					context.arc(xx-px/scale+mplanets[0].x/scale, yy-py/scale+mplanets[0].y/scale, oradius, 0, 2 * Math.PI, false); 
					context.lineWidth = 1; 
					context.stroke();	//ok now actually draw it.	
					}
				}
	...
	handlemap(myi){
		var theplayer = this.players[myi];
		if (theplayer.mapactive == 0){
			context.font = "20px Ariel";
			context.fillStyle = "white";
			context.fillText("Map is off.",16,canvas.height-16);
			}
		//else if (theplayer.mapactive == 1){drawmap(systems[ps].planets,systems[ps].outposts,theplayer.mapscale,canvas.width/2,canvas.height/2, vwx, vwy, theplayer.radarrange,systems[ps].npcs,theplayer);}//centered map
		else if (theplayer.mapactive == 1){this.drawmap(myi,canvas.width/2,canvas.height/2);}//centered map
		else if (theplayer.mapactive == 2){
			if (comfy){
				context.save();
				context.beginPath();
				context.rect(0,canvas.height-308,408,308)
				context.clip();
				...
				//drawmap(systems[ps].planets,systems[ps].outposts,theplayer.mapscale,200,canvas.height-150, vwx, vwy, theplayer.radarrange,systems[ps].npcs,theplayer);//corner map c 
				this.drawmap(myi,200,canvas.height-150);
				drawframe(0,canvas.height-300,400,300);//function drawframe(x,y,xs,ys){
				context.restore();
				}
			else{
				var mapx = 240;
				...
				//drawmap(systems[ps].planets,systems[ps].outposts,theplayer.mapscale,mapx/2,canvas.height-mapy/2, vwx, vwy, theplayer.radarrange,systems[ps].npcs,theplayer);//corner map c 
				this.drawmap(myi,mapx/2,canvas.height-mapy/2);
		...
		while (i>0){
			...
			if (this.players[i].respawning == true){
				if (this.outposts.length>0){
					this.players[i].ship.match(this.outposts[0]);//Default respawn is at station 0
					this.players[i].respawning = false;
					}
				else if (this.playerspawnplanet>=0){//If no stations, respawn in orbit of parent planet
					var parentplanet = this.planets[this.playerspawnplanet];
					var rdir = Math.random()*Math.PI*2; //random direction from planet
					var rdist = parentplanet.s+this.players[i].ship.s+12+(1+Math.random())*2*parentplanet.s; //random ish distance
					var rcw = Math.floor(Math.random()*2)*2 - 1; //random orbit direction (-1 or 1);
...
////////Bling update///////
	...
	autopilot(myi){
		var theplayer = this.players[myi];
		var theship = this.players[myi].ship;
		if (theplayer.autopilot == 1){//"autopilot"
			if (theplayer.navactive==1){theship.seek3(this.planets[theplayer.navtarget],20,30,time, 1500);}
		...
		if (theplayer.autopilot == 3){//not used
			var orbitr = 1000;
			var gravy = this.planets[theplayer.navtarget].m*.0003 / (orbitr*orbitr);  //gMm/r^2, where m is 1;
			var orbitspeed = Math.sqrt(gravy*orbitr);  //a = v^2/r, a* r = v^2, v = sqrt(a*r)
			theship.seek4(this.planets[theplayer.navtarget],20,time,orbitr,orbitspeed);
			}
		}
	hovertext(myi,vkeys,svkeys){
		var theplayer = this.players[myi]
		if (theplayer.vkvisible == true){
			var i=0;
			while(i<vkeys.length){
				if (vkeys[i].inside(mdx+canvas.width/2,mdy+canvas.height/2)){//mouseyoffset is needed to help support bad / fake fullscreen browser implementation
					var mpi = vkeys[i].key;
					var hovertext = hoverhelp(mpi);
					context.font = "20px Ariel";
					context.fillStyle = "white";
					context.fillText(hovertext,mdx+canvas.width/2 - 50,mdy+canvas.height/2 - 25);
					i=vkeys.length; //Exit loop.
					}
				i++;
				}
			var i=0;
			while((theplayer.dockstate>=0)&&(i<svkeys.length)){
				if (svkeys[i].inside(mdx+canvas.width/2,mdy+canvas.height/2)){//mouseyoffset is needed to help support bad / fake fullscreen browser implementation
					var mpi = svkeys[i].key;
					if (mpi=="ArrowUp"){var hovertext = "Previous item";}
					else if (mpi=="ArrowDown"){var hovertext = "Next item";}
					else if (mpi=="ArrowLeft"){var hovertext = "Previous menu";}
					else if (mpi=="ArrowRight"){var hovertext = "Next menu";}
					else { var hovertext = hoverhelp(mpi); }
					context.font = "20px Ariel";
					context.fillStyle = "white";
					context.fillText(hovertext,mdx+canvas.width/2 - 50,mdy+canvas.height/2 - 25);
	...
	solarhurt(myi){
		var theplayer = this.players[myi];
		if (this.solardamage==true){
			var solarignore = 0;
			var solarresist = 1;
			if (theplayer.shieldbonus == "solar"){
				solarignore = 2;
				solarresist = 0.25
				}
			theplayer.solarpain = .025*this.planets[0].m/(theplayer.ship.distance(this.planets[0])*theplayer.ship.distance(this.planets[0]))*solarresist;
			if ( theplayer.solarpain>(1.125+solarignore) ) { theplayer.ship.damage(Math.floor(8*theplayer.solarpain)/8-1-solarignore); }//Once this is settled it should move to the system update sequence.
			}
		}
	handleoutposts(playerradio){
		var i=0;//Outpost stuff all handled here.....
		while (i<this.outposts.length){ //for all outposts
			var qq = 0;
			while (qq<this.players.length){//for all players to each outpost...
				if (this.players[qq].ship.distance(this.outposts[i])<160){
					this.outposts[i].dock(this.players[qq].ship);
					this.players[qq].dockstate = i;
					if (this.players[qq].ship.shield < this.players[qq].ship.maxshield){
						this.players[qq].ship.shield = this.players[qq].ship.shield+10;
						}
					}
				qq++;
				}
			...
			while (j<this.shops[i].missions.length){//for all missions (j) from shop at outpost (i)
				var k = 0;
				while (k<this.players.length){
					var mischeck = this.shops[i].missions[j].check(this.players[k],this.npcs,this.planets,playerradio);//theships,theplanets,theradio
					if (mischeck[0]>0){
						this.players[k].money = this.players[k].money + mischeck[0];
						if (mischeck[1]>0){this.players[k].storystate = mischeck[1];}
						}
					k++;
					}
				j++;
	...
	updatespecialweapons(time,vwx,vwy){
	//for handling beam, rapid fire, and probe weapons.	
		var qq = 0;
		while(qq<this.players.length){
			var aplayer = this.players[qq];
			var aship = this.players[qq].ship; 
			//Beam weapon handling, including drawing//////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		//////////////////////////////////Rapid blasters//////////////////////////////////////////////////////////////////////////////
			...
			if ((aplayer.wep==0)&&(aplayer.blasters[0].bombs[0].timer>0)){//wep == 11 to disable, wep should ==0
				...
				while (j>1){ //for all ships...
					j=j-1;
					if (aplayer.blasters[0].bombs[0].collide(this.npcs[j].ship)){//Checks for collision only
						if (aplayer.probemode == 0){//clones color
							aship.c = this.npcs[j].ship.c;
							aship.c2 = this.npcs[j].ship.c2;
						}else if (aplayer.probemode == 1){//clones shape
							aship.polytheta = this.npcs[j].ship.polytheta;//Applies enemy polygon to player
							aship.polyradius = this.npcs[j].ship.polyradius;
						...
						//	constructor(name,description,price,type,basedamage,updamage,maxdamage,basespeed,upspeed,maxspeed,baseboom,upboom,maxboom,basen,upn,maxn,basetimer,uptimer,maxtimer,nrg,bombcolor,ID){
							//var bb = this.botbombs[j];
							aplayer.blasters[10] = fullblastercopy(this.npcs[j].blasters[0]);
				...
				while (j>1){ //for all turrets...
					j=j-1;
					if (aplayer.blasters[0].bombs[0].collide(this.turrets[j].pivot)){//Checks for collision only
						if (aplayer.probemode == 0){//clones color
							aship.c = this.turrets[j].pivot.c;
							aship.c2 = this.turrets[j].pivot.c2;
						}else if (aplayer.probemode == 1){//clones shape
							aship.polytheta = this.turrets[j].pivot.polytheta;//Applies enemy polygon to player
							aship.polyradius = this.turrets[j].pivot.polyradius;
						}else if (aplayer.probemode == 2){
						//	constructor(name,description,price,type,basedamage,updamage,maxdamage,basespeed,upspeed,maxspeed,baseboom,upboom,maxboom,basen,upn,maxn,basetimer,uptimer,maxtimer,nrg,bombcolor,ID){
							var bb = this.turrets[j].bombs[0];
...
//////////////////turret bombs hitting ships and players///////////////////////////////////////////////
	...
	playermice(){
		...
		while (qq<this.players.length){
			...
			if (((aplayer.mousestate==1)&&(aplayer.thrustmode==false))||((aplayer.mousestate==2)&&(aplayer.thrustmode==true))){ //if it's the left button
				...
				if (aplayer.wep < 20){
					//console.log("itried3");
					//console.log(theplayer.energy+"  "+theplayer.blasters[1].ecost);//theplayer.blasters[theplayer.wep].ecost);
				...
				if (aplayer.thruster>0){
					if (aplayer.cthrust){//aplayer.cthrust is a boolean flag indicating continuous instead of pulsed thrust
						aplayer.ship.cthrust = aplayer.thrustmultiplier/6;
						console.log(aplayer.thruster);
						//console.log("itriedtocthrust");
						}
					else {
						aplayer.ship.thrust = 2*aplayer.thrustmultiplier;
						}
					//theplayer.thruster = theplayer.thruster - 24;
					//var td = 48;
					//var tr = 24;
					...
					aplayer.ship.cthrust = 0; //this turns off the continuous thruster if no thruster energy is available.
					}
				} 
			...
			switch (aplayer.input) {  //events for all the keyboard keys
				...
				case ")": 
					if (aplayer.blasters[10].phas){ aplayer.wep = 10; }
					  break;
				case "n": 
					//console.log(aplayer.upgrades[7]);
					if (aplayer.navactive == 0){
						...
						if (aplayer.upgrades[7].tier>0){ aplayer.navactive = 3; }
						else { aplayer.navactive = 0; }
					} else if (aplayer.navactive == 3){
						aplayer.navactive = 0;
						}
					  break;
				case "m": 
				...
				case ".": 
					var maxnavtarget = this.planets.length-1;
					//waldo provision removed.
					//if ((aplayer.sensor<1)&&(this.planets[this.planets.length-1].s<64)){maxnavtarget--;}//Player can't select waldo, the last planet, without player.sensor >=1;
					if (aplayer.navactive == 1){
						aplayer.navtarget++;
						if (aplayer.navtarget > maxnavtarget){ aplayer.navtarget = 0; }//these are probably redundant
					}else if (aplayer.navactive == 2){
						aplayer.navtarget++;
						if (aplayer.navtarget > this.outposts.length-1){aplayer.navtarget = 0; }//these are probably redundant
					}else if (aplayer.navactive == 3){
						aplayer.navtarget++;
				...
				case "z":
				 ...
				 if (cheatmode == 1){//global scope stuff
					...
					aplayer.ship.vy = 0;
					//theplayer.planetarychart = systems[ps].generateplanetlist();
					}
				  break;
				  case "s":
				 ...
				 case "p": 
				  ...
				  break;
				  case "azt": 
					if (aplayer.autozoom==1){aplayer.autozoom = 0;}//Global scope, shame
					else {aplayer.autozoom = 1;}
				  break;
				   case "k": 
				   //save game
	...
	randomplanets(){
		...
		while (i<numplanets-1){
			...
			while (i<this.planets.length){
				...
				if (proximity < 8*(this.planets[i].s+planetsize+orbitradius/100)){
					orbitradius = Math.floor( (Math.random()*Math.random()*Math.random()*250000) + 4*this.planets[0].s);//rerandomizes orbit
	...
	randomplanets2(){
		var numplanets = Math.floor(Math.random()*8+8);//random number of planets, 5-11
		var orbitradius = 0; //randomized in the loop
		var planetsize = 0; //randomized in the loop
		this.planets.push( new Umo(this.x,this.y,Math.floor(Math.random()*3000+1000), "orange") ); //make the sun 
		this.planets[0].name = this.name; // Star name is same as system name
		var numinners = Math.floor(2+Math.random()*4);
	...
	randommoons(index){//index is of planet
		...
		if (this.planets[0].distance(this.planets[index])<this.planets[0].s*10){nummoons = 0;}
		...
		while (i>0){
			...
			if (this.planets[0].distance(this.planets[index])>moonorbitr*12){
	...
	randomoutposts(num){//num is number of outposts
		...
		var i=0;
		while(i<this.planets.length){
			if ((this.planets[i].parentid==0)&&(this.planets[i].distance(this.planets[0])>25000)){//not too close to sun, no moons allowed
				eligibleplanets.push(i);
				}
			i++;
			}
		//console.log(eligibleplanets);
		if (eligibleplanets.length<num){
			//console.log("ERROR NOT ENOUGH ELIGIBLE PLANETS ("+eligibleplanets.length+") FOR OUTPOSTS ("+num+").");
			}
		var pickedplanets = [];
		while((pickedplanets.length<num)&&(eligibleplanets.length>0)){
			var pick = Math.floor(Math.random()*eligibleplanets.length);//choose 1 from eligible planets
			//console.log(pick);
			//console.log(eligibleplanets[pick]);
			pickedplanets.push(eligibleplanets[pick]);//add to picked planets
			eligibleplanets.splice(pick, 1);//remove choice from eligible planets so it won't get reused
			//console.log(eligibleplanets);
			//console.log(pickedplanets);
			}
		//console.log(pickedplanets);
		//var alreadypickedplanets = [];
		var i=0;
		while (i<pickedplanets.length){//
			...
			var pickedplanet = pickedplanets[i];//Math.floor(Math.random()*(this.planets.length-2))+1;//station will be in a solar following orbit to the chosen planet
			...
			var minangle = (this.planets[pickedplanet].s*16) / (orbitdistance);
			this.outposts[lastindex].setorbit(this.planets[0], orbitdistance, orbitposition+minangle+Math.random()*0.1, 1);//This properly sets orbital distance, maybe properly sets orbit position.
			...
			var j=0;//Remove duplicate shop items
			while(j<randshopitems3.length){
				var k = j+1;
				while(k<randshopitems3.length){
					if (randshopitems3[j].describestring()==randshopitems3[k].describestring()){
						randshopitems3.splice(k,1);//remove the duplicate
						k--; //decrement to compensate for shortening the list.
						}
					k++
					}
				j++;
			...
			this.shops.push(new Shop(this.outposts[lastindex].name,i, "randomized descriptions should go here", randshopitems3)); //should fix this to reflect outpost name/description
			var k = 0;
	...
	generateasteroidbelt(r,w,s,n){//radius, width, size, number
		var i=0;
		while(i<n){
			var arock = new Umo(0,0,s,"grey");
			arock.setorbit(this.planets[0], r, Math.random()*6.14, 1);//setorbit(parentplanet, distance, direction, cw){ //cw = -1 or 1
			arock.parentid = this.planets.length-1;//kludgey way of dodging a mess of orbit circles on radar
			arock.c2 = "white";
			i++;
			}
		}
	...
	enemypopulate(num,minlevel,maxlevel){ //Adds gangs of enemy ships, level describes difficulty, num is size of each gang.
		var i=1;
		while (i<this.planets.length-1){//maybe not exclude last planet?
			var templevel = Math.floor(minlevel + Math.random()*(maxlevel+1 - minlevel));
			var pdistance = this.planets[i].distance(this.planets[0]);
			if (pdistance<12000){
				this.addplanetturrets(i,Math.floor(Math.random()*6)+2,128+32*templevel,32+16*templevel);
				}
			else {
	...
	friendlypopulate(numtraders,nummercs,gangsize,minlevel,maxlevel){ //Adds gangs of enemy ships, level describes difficulty, num is size of each gang.
		...
		while (i<numtraders){
			...
			if (numdestinations<this.planets.length-2){
				var j=0;
			...
			else { console.log("Not enough planets "+this.planets.length+" for trader stops "+numdestinations); }
		...
		while (i<nummercs){
			...
			else { console.log("Not enough planets "+this.planets.length+" for merc stops "+numdestinations); }
			//addrandomtraders(destinations, num, level){
			i++;
			}
		}
	generatebasicsystem(gangsize,minlevel,maxlevel,bling){
		...
		this.friendlypopulate(3,1,Math.floor(gangsize/2)+1,minlevel,maxlevel);//friendlypopulate(numtraders,nummercs,gangsize,minlevel,maxlevel){ 
	...
	generatebasicsystem2(gangsize,minlevel,maxlevel,bling){
		this.randomplanets2();
		//var asgap = this.findgap();
		//this.generateasteroidbelt(asgap[1],asgap[0]/2,50,24);//radius, width, size, number
		this.enemypopulate(gangsize,minlevel,maxlevel);
		this.friendlypopulate(3,1,Math.floor(gangsize/2)+1,minlevel,maxlevel);//friendlypopulate(numtraders,nummercs,gangsize,minlevel,maxlevel){ 
		this.addcargosales(cargoitems);//global scope shame
		this.addrandombling(bling);
	...
	generateasteroidsystem(gassers,rockers,assers,moons){//gas giants, asteroids, moons
		var astar = new Umo(0,0,2000,"yellow");
		astar.name = randname(4);
		this.planets = [astar];
		var gasorbits = []
		var i=0;
		while(i<gassers){//places gas / large outer planets
			var agassize = 750+Math.floor(500*Math.random());
			var agasr = 90000+40000*i+Math.floor(Math.random()*20000);
			var agasdir = Math.random()*6.14;
			var agas = new Umo(0,0,agassize,randcolor());
			gasorbits.push([agasr,agasdir]);
			agas.c2 = randcolor();
			agas.name = randname(4);
			agas.parentid = 0;
			agas.setorbit(this.planets[0],agasr, agasdir, 1);//setorbit(parentplanet, distance, direction, cw){ //cw = -1 or 1
			this.planets.push(agas);
			i++;
			}
		var i=0;
		while (i<rockers){//places rocky / small inner plannets
			var arockersize = 128+Math.floor(Math.random()*256);
			var spacing = 10000//Math.floor(30000/rockers);
			var arockerr =8000+spacing*i+Math.floor(Math.random()*spacing/2);
			var arockerdir = Math.random()*6.14;
			var arocker = new Umo(0,0,arockersize,randcolor());
			arocker.c2 = randcolor();
			arocker.name = randname(4);
			arocker.parentid = 0;
			arocker.setorbit(this.planets[0],arockerr, arockerdir, 1);//setorbit(parentplanet, distance, direction, cw){ //cw = -1 or 1
			//console.log(arocker);
			this.planets.push(arocker);
			i++;
			}
		var i=1;
		var numplanets = this.planets.length;
		while (i<numplanets-1){//give planets moons as appropriate.  -1 seems apocryphal
			this.randommoons(i); //randommoons function decides how many moons for each planet based on size
			i=i+1;
			}
		var i = 0;
		while (i<this.planets.length){//applies extra dots and randomizes dot size and location
			var extradots = Math.floor(Math.random()*3);//Default is 3 dots, this adds 0 to 2 more dots.
			while(extradots>0){
				this.planets[i].polyradius.push(0);
				this.planets[i].polytheta.push(0);
				extradots--;
			...
			var j=0;
			while (j<this.planets[i].polytheta.length){
				this.planets[i].polyradius[j] = Math.random()+0.125;//This number is interpreted as a dot size for planet rendering.
				this.planets[i].polytheta[j] = Math.random()*2*Math.PI;
				j=j+1;
				}
		...
		this.randomoutposts(gassers); //add a number of outposts equal to the number of gassers
		var i=0;
		while(i<gasorbits.length){//now place the outposts in orbits trailing each gasser
			this.outposts[i].setorbit(this.planets[0],gasorbits[i][0], gasorbits[i][1]+0.25, 1);//setorbit(parentplanet, distance, direction, cw){ //cw = -1 or 1
			i++;
			}
		var traderstops = Math.floor(Math.random()*3)+2;//generating NPC trader stops 
		var destinations = [];
		var i=0;
		while (i<traderstops){
			var thisstop = Math.floor(Math.random()*(this.planets.length-1))+1;
		...
		this.addrandomtraders(destinations, 4, 15); //adds a group of 4 level 15 traders with previously determined destinations
		var traderstops = Math.floor(Math.random()*3)+2; //this is duplicated destination code for a 2nd group
		var destinations = [];
		var i=0;
		while (i<traderstops){
			var thisstop = Math.floor(Math.random()*(this.planets.length-1))+1;
		...
		var asgap = this.findgap(); //find best place for asteroids
		this.generateasteroidbelt(asgap[1],asgap[0]/3,48,assers);//radius, width, size, number
		
		this.enemypopulate(3,2,16);
		this.friendlypopulate(3,1,2,8,16);//friendlypopulate(numtraders,nummercs,gangsize,minlevel,maxlevel){ 
		this.addcargosales(cargoitems);//global scope shame
		this.addrandombling(80);
		this.addsuperboss(96,4,1000+4*128,500+4*32,320,80,1+Math.floor((this.planets.length-2)*Math.random()));
		this.addsuperboss(96,6,1000+8*128,500+8*32,320,80,1+Math.floor((this.planets.length-2)*Math.random()));
		}		
	generateasteroidbelt(r,w,s,n){//radius, width, size, number
		var i=0;
		while(i<n){
			var arock = new Umo(0,0,Math.floor(s*(0.5+Math.random()*1.5)),"grey");
			arock.name = randname(4);
			arock.setorbit(this.planets[0], r+(0.5-Math.random())*w, Math.random()*6.14, 1);//setorbit(parentplanet, distance, direction, cw){ //cw = -1 or 1
			arock.parentid = this.planets.length-1;//kludgey way of dodging a mess of orbit circles on radar
			arock.c2 = "white";
			this.planets.push(arock);
			i++;
			}
		}
	generateminisystem(rockers){//gas giants, asteroids, moons
		var astar = new Umo(0,0,2000,"yellow");
		astar.name = randname(4);
		//console.log("1");
		this.planets = [astar];
		...
		var numoutposts = 1 + Math.floor(rockers/2);
		this.randomoutposts(numoutposts); //add a number of outposts equal to the number of gassers
		/*
		var traderstops = Math.floor(Math.random()*3)+2;//generating NPC trader stops 
		console.log("5");
		var destinations = [];
		...
		*/
		...
		this.enemypopulate(3,2,16);
		//console.log("8.3");
		this.friendlypopulate(3,1,2,8,16);//add friendly NPCs that fight the bots.
		//console.log("8.5");
		this.addcargosales(cargoitems);//global scope shame
		this.addrandombling(80);
		this.addsuperboss(96,4,1000+4*128,500+4*32,320,80,1+Math.floor((this.planets.length-2)*Math.random()));
	...
	findgap(){
		...
		var i=1;
		while(i<this.planets.length){
			if (this.planets[i].parentid==0){
				orbits.push(this.planets[i].distance(this.planets[0]));
				}
			i++
			}
		...
		//console.log(orbits);
		orbits.sort(compareNumbers);
		//console.log(orbits);
		var i=1;
		while(i<orbits.length){
			var agap = orbits[i]-orbits[i-1];
			if (agap>bestgap){
				bestgap = agap;
				bestgapr = (orbits[i]+orbits[i-1])/2;
				}
			i++;
			}
		return [bestgap,bestgapr];
		}
	...
	addplanetturrets(planeti,turretnum,turrethp,turretshield){
		var randomturretverts = randpolarpoly(16, 0.5);//sides,  minimum radius
		normalizepoly(randomturretverts);
		var tc1 = randcolor();
		var tc2 = randcolor();
		var i=0;
		while(i<turretnum){
			var pb = this.planets[planeti];
			var newturret = new Turret("enemy",pb,(i/turretnum)*6.14,pb.s+32);
			newturret.pivot.hp = turrethp;
			newturret.pivot.maxhp = turrethp;
			newturret.pivot.shield = turretshield;
			newturret.pivot.maxshield = turretshield;
