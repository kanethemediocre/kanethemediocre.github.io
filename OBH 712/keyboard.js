...
window.addEventListener("keydown", function (event) {
  if (event.defaultPrevented) {
	...
	if((myplayer.emenu>0)&&(myplayer.alive)){
		...
		var myscore = 0;
		var i=0;
		while(i<myplayer.deadtree.emods[0].quizblocks[0].quizzes[0].challenges.length){
			if (myplayer.deadtree.emods[0].quizblocks[0].quizzes[0].challenges[i].answer == myplayer.deadtree.emods[0].quizblocks[0].quizzes[0].challenges[i].playeranswer){
				myscore++;
				}
			i++;
			}
		if (myscore>myplayer.deadscore){
			myplayer.deadscore = myscore;
			myplayer.deadtime = myplayer.deadtime - 400;
			}
		console.log(myscore);
	...
	else if ((event.key == "u")&&(cheatmode==1)){//uio were used for experiments with adding extra players
		//myi++;
